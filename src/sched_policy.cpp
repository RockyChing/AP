...
{
    if (fd < 0) {
        LOG("add_tid_to_cgroup failed; fd=%d\n", fd);
        errno = EINVAL;
        return -1;
    }
    ...
    *ptr = '\0';
    ...
    if (write(fd, ptr, end - ptr) < 0) {
        ...
        if (errno == ESRCH)
                return 0;
        LOG("add_tid_to_cgroup failed to write '%s' (%s); fd=%d\n",
...
static int getSchedulerGroup(int tid, char* buf, size_t bufLen)
{
    ...
    LOG("Failed to find cpu subsys");
    fclose(fp);
    return -1;
 out_bad_data:
    LOG("Bad cgroup data {%s}", lineBuf);
    fclose(fp);
    return -1;
}
